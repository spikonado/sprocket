...
`threads:settle` and `threads:unsettle` mutations. Older servers also subscribe
to `threads:listRecent` for their on-disk summary cache. Keep that query until
the same minimum-version gate passes.
