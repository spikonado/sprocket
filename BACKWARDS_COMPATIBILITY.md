...
## Live leftover: `chat.latestRunForThread` and direct `finalizeRun` cancel

Current UI reads `chat.selectedThreadLifecycle` and writes cancellation with
`agentRuntime.requestCancellation`. Older clients still call
`chat.latestRunForThread` (full run document, active job, prompt) and cancel
by `agentRuntime.finalizeRun({ status: 'cancelled' })`, which terminalizes
immediately.

Keep both until released desktops/agents stop calling them. Then delete the
query and stop treating user-facing `finalizeRun` as a cancel path.
