...
mod tests {
    ...
    #[test]
    fn watch_request_accepts_project_only_and_valid_threads() {
        let project: ArtifactWatchRequest = serde_json::from_value(serde_json::json!({
            "userId": "user-1",
            "repositoryKey": "repo",
            "workspacePath": "/workspace"
        }))
        .unwrap();
        assert!(normalize_thread_id(project.thread_id.as_deref()).is_none());

        let empty_thread: ArtifactWatchRequest = serde_json::from_value(serde_json::json!({
            "userId": "user-1",
            "repositoryKey": "repo",
            "workspacePath": "/workspace",
            "threadId": ""
        ...
        .unwrap();
        assert!(normalize_thread_id(empty_thread.thread_id.as_deref()).is_none());

        let thread: ArtifactWatchRequest = serde_json::from_value(serde_json::json!({
            "userId": "user-1",
            "repositoryKey": "repo",
            "workspacePath": "/workspace",
            "threadId": "thread-1"
        ...
        .unwrap();
        assert_eq!(
            normalize_thread_id(thread.thread_id.as_deref()),
            Some("thread-1")
        );

        let blobs: ArtifactWatchRequest = serde_json::from_value(serde_json::json!({
            "userId": "user-1",
            "repositoryKey": "repo",
            "workspacePath": "/workspace",
            "threadId": "blobs"
        ...
        .unwrap();
        assert_eq!(
            normalize_thread_id(blobs.thread_id.as_deref()),
            Some("blobs")
        );

        let traversal: ArtifactWatchRequest = serde_json::from_value(serde_json::json!({
            "userId": "user-1",
            "repositoryKey": "repo",
            "workspacePath": "/workspace",
            "threadId": "../other"
        ...
        .unwrap();
        assert_eq!(
            normalize_thread_id(traversal.thread_id.as_deref()),
            Some("../other")
        );
    }
}
