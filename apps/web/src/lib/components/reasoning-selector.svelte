<script lang="ts">
	...
	let showsReasoning = $derived(showsReasoningControl(model));
	let showsFastModeControl = $derived(
		fastModeAccess === 'available' || fastModeAccess === 'locked'
	);
	...
	$effect(() => {
		if (disabled || (!showsReasoning && !showsFastModeControl)) isOpen = false;
	});
</script>

{#if showsReasoning || showsFastModeControl}
